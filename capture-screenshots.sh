...
OUT_DARK="screenshots/kopilka-main-dark.png"
WINDOW_W=1200
WINDOW_H=800
...
# that keyfile never lands in Cal's real ~/.config; Kopilka resolves its own
...
KEYFILE
  ...
  echo "==> Launching Kopilka ($scheme) against demo data inside the isolated display"
  env -u WAYLAND_DISPLAY GDK_BACKEND=x11 HOME="$DEMO_HOME" XDG_CONFIG_HOME="$DEMO_HOME/.config" \
    ADW_DISABLE_PORTAL=1 GSETTINGS_BACKEND=keyfile DISPLAY=":$DISPLAY_NUM" python3 -m kopilka.kopilka &
  APP_PID=$!

#!/usr/bin/env bash
# dev-build.sh — build and install Kopilka locally for testing
#
...
MANIFEST="io.github.calstfrancis.kopilka.yml"

VERSION=$(grep '^version' pyproject.toml | head -1 | sed 's/version = "\(.*\)"/\1/')
echo "==> Building Kopilka $VERSION (local dev install)"
...
echo ""
echo "Done! Kopilka $VERSION is installed locally."
echo "Run it with: flatpak run io.github.calstfrancis.kopilka"
